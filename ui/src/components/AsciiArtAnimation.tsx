...
type EllieSprite = (typeof ELLIE_SPRITES)[number];
...
function spriteSize(sprite: EllieSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = ELLIE_SPRITES[(baseCol + baseRow) % ELLIE_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = ELLIE_SPRITES[Math.floor(Math.random() * ELLIE_SPRITES.length)]!;
